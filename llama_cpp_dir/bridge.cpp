...
struct pyllama_cpuBuffer {
    pyllama_cpuBuffer(gpt_vocab const& vocab, std::size_t len, Fn&& fn)
        : m_vocab(vocab)
        , max_len(len)
        , m_fn(std::move(fn))
    {}
...
struct pyllama_cpu {
    ...
    pyllama_cpu(std::string const& path, int num_threads, int n_ctx, std::size_t last_n_size, int seed)
    ...
    bool generate(std::function<void(std::string const&)> fn, std::size_t num_tokens, float top_p, float temp, float repeat_penalty, std::string stop_word = nullptr) {
        ...
        auto buffer = pyllama_cpuBuffer(m_vocab, stop_word.size() + 1, [&fn](std::string const s) {
            fn(s);
        });
...
PYBIND11_MODULE(pyllama_cpu, m) {
    py::class_<pyllama_cpu>(m, "Model")
        ...
        .def("ingest", &pyllama_cpu::ingest, py::arg("prompt"))
        .def("generate", &pyllama_cpu::generate, py::arg("streaming_fn"), py::arg("num_tokens") = 100, py::arg("top_p") = 0.95f, py::arg("temp") = 0.8f, py::arg("repeat_penalty") = 1.f, py::arg("stop_word") = "")
        .def("save_state", &pyllama_cpu::save_state, py::arg("path"))
        .def("load_state", &pyllama_cpu::load_state, py::arg("path"));
}